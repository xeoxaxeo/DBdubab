...
public class CLIApplication {
    ...
    public static void main(String[] args) {
        ...
        try (Connection conn = DriverManager.getConnection(URL, USER, PASS)) {
            switch (action) {
                case "active" -> new PharmacyDAO(conn).findActivePharmaciesNow();
                ...
                case "regionActive" -> {
                    if (args.length < 2) {
                        ...
                        new PharmacyDAO(conn).findActivePharmaciesByRegion(args[1]);
                    }
                }
                ...
                case "operatingRank" -> {
                    if (args.length < 2) {
                        ...
                        new PharmacyDAO(conn).findPharmaciesByOperatingTimeWithRank(args[1]);
                    }
                }
                ...
                case "detail" -> {
                    if (args.length < 2) {
                        ...
                        new PharmacyDAO(conn).getPharmacyDetails(args[1]);
                    }
                }

                case "longhours" -> new PharmacyDAO(conn).getLongOperatingPharmacies();

                // rollup
                // jaeeun -- 수정 사항

                case "rollup" -> new RollupManager(conn).showRegionDayRollup();
    ...
    private static void printUsage() {
        ...
        System.out.println("  stats [minCount]                       → 지역/요일별 약국 수 통계");
        System.out.println("  regionActive <region>                  → 지정 지역 운영 약국 조회");
        ...
        System.out.println("  emergency-delete <id>                  → 응급약국 삭제");
        System.out.println("  pharmacy-insert <...>                  → 약국 등록");
        System.out.println("  pharmacy-update <...>                  → 약국 수정");
        System.out.println("  pharmacy-delete <id>                   → 약국 삭제");
    }
}
